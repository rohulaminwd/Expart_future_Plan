    ...
    <div className="w-full sm:p-0 p-2">
        ...
        <div className="relative border-[#6182c9] sm:py-8 sm:px-8 px-3 py-5 bg-[#263186ce] text-white rounded-2xl">
          <div className="flex gap-3 sm:gap-5 items-center">
            <div className="sm:pl-8 pl-2">
              <div className="w-16 h-16 sm:w-44 sm:h-44 bg-primary rounded-full flex items-center justify-center ring ring-[#91f2dc] ring-offset-base-100 ring-offset-2">
                <h2 className="text-3xl uppercase sm:text-8xl font-bold text-white">
            ...
            <div className="text-white">
              <h2 className="sm:text-5xl text-[24px] sm:my-3">
                {me?.firstName} {me?.lastName}
              </h2>
              <h2 className="sm:text-2xl text-[16px] text-gray-300">
                {me?.phoneNumber}
              </h2>
              {me?.email && (
                <h2 className="sm:text-2xl text-[12px] text-gray-300">
                  {me?.email}
                </h2>
              )}
            </div>
          ...
          <div className="flex w-full mt-5 sm:mt-8 items-center justify-between">
            <div className="w-full">
              <Link
                to="/dashboard/planDetails"
                className="btn text-white sm:btn-wide sm:btn-md btn-sm rounded-full btn-accent"
              >
                Active Plan
              </Link>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-center mt-5 sm:mt-8 justify-between w-full gap-3">
        <label
          ...
          for="update-password"
          className="rounded-r-xl cursor-pointer border-l-[5px] border-[#a145f6] flex items-center py-1 bg-white justify-between shadow-md"
        >
          <div className="flex items-center gap-2">
            ...
            <div className="">
              ...
              <p className="text-[10px]">
                অবশ্যই আপনাদের একাউন্ট নম্বর টি যুক্ত করে নিবেন উইথড্র রিকুয়েষ্ট
                করার আগে।
              </p>
            </div>
          </div>
        ...
        <label
          onClick={() => setUpdateModal([me, "account"])}
          for="update-password"
          className="rounded-r-xl cursor-pointer border-l-[5px] border-[#a145f6] flex items-center py-1 bg-white justify-between shadow-md"
        >
          <div className="flex items-center gap-2">
            ...
            <div className="">
              ...
              <p className="text-[10px]">
                আপনি চাইলে এখানে থেকে পাসওয়ার্ড পরিবর্তন করতে পারেন।
              </p>
            </div>
          </div>
          <div>
      ...
      <div className="w-full mt-3 sm:mt-5 p-2 py-3 shadow-md bg-[#caf8ec] rounded-r-lg border-l-[5px] border-primary flex items-center gap-2">
        ...
        {me?.bkash ? (
          <h2 className="sm:text-2xl">{me?.bkash}</h2>
        ) : (
          <p>Account is not available</p>
        )}
      </div>
      <div className="w-full mt-3 sm:mt-5 p-2 py-3 shadow-md bg-[#cef8ed] rounded-r-lg border-l-[5px] border-primary flex items-center gap-2">
        ...
        {me?.nagad ? (
          <h2 className="sm:text-2xl">{me?.nagad}</h2>
        ) : (
          <p>Account is not available</p>
        )}
      </div>
