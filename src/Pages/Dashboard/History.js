        ...
        <div className='w-full sm:p-0 p-2'>
            <div className='w-full bg-white p-2 sm:p-3 rounded-md shadow-md'>
                <div className="w-full flex items-center justify-between ">
                    <div onClick={() => handleState(pending, 'Recharge1')} className={`${(design === 'Recharge1')? 'bg-gradient-to-r from-[#13b38f] to-[#2091d9] duration-100 rounded-md !text-white' : 'border bg-slate-100'} cursor-pointer w-full py-2 px-0`}>
                        <div className='text-center'>
                            <h1 className='text-xl sm:text-2xl'>{pending?.length}</h1>
                            <h1 className='text-[14px]'>Pending</h1>
                        </div>
                    </div> 
                    <div onClick={() => handleState(complete, 'Recharge2')} className={`${(design === 'Recharge2')? 'bg-gradient-to-r from-[#13b38f] to-[#2091d9] text-white duration-100 rounded-md' : 'border bg-slate-100'} cursor-pointer w-full py-2 px-0`}>
                        <div className='text-center'>
                            <h1 className='text-xl sm:text-2xl'>{complete?.length}</h1>
                            <h1 className='text-[14px]'>Complete</h1>
                        </div>
                    </div> 
                    <div onClick={() => handleState(cancelled, 'Recharge3')} className={`${(design === 'Recharge3')? 'bg-gradient-to-r from-[#13b38f] to-[#2091d9] text-white duration-100 rounded-md' : 'border bg-slate-100'} cursor-pointer w-full py-2 px-0`}>
                        <div className='text-center'>
                ...
                <div className='md:mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-5 mt-3'>
                    {
                        ...
                        className={`${(i?.status === 'pending')? "hover:border-[#c9f8d6] text-[#000] bg-white" : "bg-[#ffffff] text-[#000] hover:border-[#dedede]"} 
                        ${(i?.status === 'cancelled')? "hover:border-[#399d2c] bg-red-900 text-white" : "text-white hover:border-[#9df1e5]"}  
                        p-2 md:mt-0 sm:p-3 w-full cursor-pointer border-[1px]  duration-300 shadow-md rounded-2xl hover:!text-white hover:bg-gradient-to-r hover:from-[#13b38f] hover:to-[#2091d9]`}>
                            <div className='flex relative border-b pb-2 justify-between gap-2'>
                                <div className='w-full flex items-center gap-2'>
                                    <div>
                                        {(i.sector === 'recharge') && <img src={recharge} className='w-10' alt="pending " />}
                                        {(i.sector === 'withdraw') && <img src={withdraw} className='w-10' alt="pending " />}
                                    </div>
                                    <div>
                                        <h3 className={` ${(i?.sector === "recharge")? "text-[#3b7a53] hover:text-white" : ""} text-[16px] capitalize font-bold`}>{i?.sector}</h3>
