...
const PlanDetails = () => {
  ...
  const isDateExpire = (PlanDate) => {
    ...
    if (expire < 0 && me?.FreePlan === "active") {
      fetch(
        `https://efp-usa-server-site.vercel.app/api/v1/User/FreePlanInactive/${me?._id}`,
        {
          method: "PATCH",
          headers: {
  ...
  const PlanDateExpire = (id) => {
    ...
    if (expire < 0) {
      fetch(
        `https://efp-usa-server-site.vercel.app/api/v1/User/planDelete/${me?._id}`,
        {
          method: "PATCH",
          headers: {
            "content-type": "application/json",
  ...
  const lifeTimePlan = plan?.find((i) =>
    i?.category?.includes("Life time Plan")
  );
              ...
              <>
                <motion.div
                  ...
                  className={`
                        ${
                          planTime === "7-Days"
                            ? "text-[#156c65] border-[#abf98d] bg-[#dbfbd7]"
                            : "bg-primary text-white border-[#9df1e5]"
                        } 
                        ${
                          planTime === "14-Days"
                            ? "text-[#156c65] border-[#8df3f9] bg-[#c2f7f6]"
                            : "bg-primary text-white border-[#9df1e5]"
                        } 
                        ${
                          planTime === "30-Days"
                            ? "text-white border-[#81a7fa] bg-[#2d4069]"
                            : "bg-primary text-white border-[#9df1e5]"
                        } 
                ...
                >
                  <div className="relative border-b pb-1">
                    ...
                    <div className="text-right flex items-center justify-end">
                      ...
                      {isActive(i?._id) === i?._id && (
                        <p className="text-[12px]">
                          Expire Date: {isPlanDate(i?._id)}
                        </p>
                      )}
                    </div>
                  </div>
