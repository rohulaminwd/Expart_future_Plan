...
const StatusUpdate = ({ setUpdateStatus, updateStatus }) => {
  const { i, refetch, request, done } = updateStatus;
  ...
  const handleUpdate = () => {
    const dataInfo = {
      ...
      amount: i?.amount,
      sector: i?.sector,
      tranId: done === "done" ? transCode : i?.tranId,
      accountNumber: done === "done" ? accountNum : i?.accountNumber,
    };
        ...
        <div className="modal-box p-3 py-5 sm:p-4">
          ...
          <div className="text-center">
            ...
            <div>
              <h2 className="text-2xl font-bold text-[#000]">
                Are you sure {i?.name} {i?.sector}{" "}
                {request === "return" ? "Cancel and Return" : "Complete"}
          ...
          <div className="flex items-center justify-center gap-3 mt-5">
            <button
              ...
              disabled={done === "done" && !transCode}
            >
              Yes
            </button>
            <label htmlFor="update-status" className="btn btn-sm w-[100px] ">
              No
            </label>
          </div>
        </div>
